...
func (b *backend) verifyCreds(ctx context.Context, req *logical.Request, client, key string) (*verifyResp, error) {
	...
	// Parse TTLs
	// old onde
  // ttl, maxTTL, err := framework.SanitizeTTLStr(config.TTL.String(), config.MaxTTL.String())
  ttlStr, ok := req.Data["ttl"].(string)
  if !ok {
	  ttlStr = ""
  }

  ttl, err := SanitizeTTLStr(ttlStr, b.Backend.System())
  if err != nil {
